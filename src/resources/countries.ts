...
const RESTRICTED_REGION_NAMES = new Set([
    "Crimea",
    "Crimea (Ukraine)",
    "Darfur",
    "Darfur (Sudan)",
...
export const ALLOWED_COUNTRIES = ALL_COUNTRIES.filter(
    (country) =>
        !RESTRICTED_COUNTRY_CODES.has(country.code) &&
        !RESTRICTED_REGION_NAMES.has(country.name)
).sort((a, b) => a.name.localeCompare(b.name));

export const COUNTRY_OPTIONS = ALLOWED_COUNTRIES.map((country) => ({
    label: country.name,
    value: country.name,
}));

const ALLOWED_COUNTRY_NAMES = new Set(ALLOWED_COUNTRIES.map((country) => country.name.toLowerCase()));
const ALLOWED_COUNTRY_CODES = new Set(ALLOWED_COUNTRIES.map((country) => country.code));

export function isAllowedCountry(value: string): boolean {
    const normalized = value.trim();
    if (!normalized) return false;

    return (
        ALLOWED_COUNTRY_NAMES.has(normalized.toLowerCase()) ||
        ALLOWED_COUNTRY_CODES.has(normalized.toUpperCase())
    );
}
